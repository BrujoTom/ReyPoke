...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-border-subtle/80", className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-5 text-left font-display text-lg font-semibold text-foreground transition-all hover:text-teal-dark focus-ring [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
